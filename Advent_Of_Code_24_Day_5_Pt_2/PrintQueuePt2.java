...
public class PrintQueuePt2 {
    ...
    public static int middleSumOfReordered(ArrayList<ArrayList<Integer>> pairs, ArrayList<ArrayList<Integer>> lists) {
        int sum = 0;

        for (int i = 0; i < lists.size(); ++i) {
            if (!isOrdered(pairs, lists.get(i))) {
                int l = 0;
                while (l < lists.get(i).size()) {
                    for (int j = 0; j < pairs.size(); ++j) {
                        int firstIndex = -1;
                        int secondIndex = -1;
            
                        if (lists.get(i).get(l).equals(pairs.get(j).get(0)) && 
                            lists.get(i).contains(pairs.get(j).get(0)) && 
                            lists.get(i).contains(pairs.get(j).get(1))) {
                            for (int k = 0; k < lists.get(i).size(); ++k) {
                                if (lists.get(i).get(k).equals(pairs.get(j).get(0))) {
                                    firstIndex = k;
                                }
                                else if (lists.get(i).get(k).equals(pairs.get(j).get(1))) {
                                    secondIndex = k;
                                } 
                            }
            
                            if (firstIndex > secondIndex) {
                                int removed = lists.get(i).get(firstIndex);
                                lists.get(i).remove(firstIndex);
                                lists.get(i).add(secondIndex, removed);
                                l = secondIndex;
                            }
                        }
                    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(middleSumOfReordered(pairs, lists));
    }
